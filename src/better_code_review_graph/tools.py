...
def _resolve_query_target(
    store, target: str, root, pattern: str
) -> tuple[Any, str, dict[str, Any] | None]:
    """Resolve target to a node, qualified name, or error response."""
...
def _handle_callers_of(
    store, node, qn: str, results: list[dict], edges_out: list[dict]
):
    qns = []
...
def _handle_callees_of(store, qn: str, results: list[dict], edges_out: list[dict]):
    qns = []
...
def _handle_imports_of(store, qn: str, results: list[dict], edges_out: list[dict]):
    for e in store.get_edges_by_source(qn):
...
def _handle_importers_of(
    store, node, target: str, root, results: list[dict], edges_out: list[dict]
) -> dict[str, Any] | None:
...
def _handle_children_of(store, qn: str, results: list[dict]):
    qns = []
    for e in store.get_edges_by_source(qn):
        if e.kind == "CONTAINS":
            qns.append(e.target_qualified)
...
def _handle_tests_for(store, node, qn: str, target: str, results: list[dict]):
    qns = []
    for e in store.get_edges_by_target(qn):
        if e.kind == "TESTED_BY":
            qns.append(e.source_qualified)
...
def _handle_inheritors_of(store, qn: str, results: list[dict], edges_out: list[dict]):
    qns = []
    for e in store.get_edges_by_target(qn):
...
def _handle_file_summary(
    store, target: str, root, results: list[dict]
) -> dict[str, Any] | None:
    full_target_raw = root / target
    full_target = full_target_raw.resolve()
    if (
...
def query_graph(
    ...
    try:
        ...
        node, target, error = _resolve_query_target(store, target, root, pattern)
        if error:
            return error

        qn = node.qualified_name if node else target

        # Dispatch to handler based on pattern
        if pattern == "callers_of":
            _handle_callers_of(store, node, qn, results, edges_out)
        elif pattern == "callees_of":
            _handle_callees_of(store, qn, results, edges_out)
        elif pattern == "imports_of":
            _handle_imports_of(store, qn, results, edges_out)
        elif pattern == "importers_of":
            err = _handle_importers_of(store, node, target, root, results, edges_out)
            if err:
                return err
        elif pattern == "children_of":
            _handle_children_of(store, qn, results)
        elif pattern == "tests_for":
            _handle_tests_for(store, node, qn, target, results)
        elif pattern == "inheritors_of":
            _handle_inheritors_of(store, qn, results, edges_out)
        elif pattern == "file_summary":
            err = _handle_file_summary(store, target, root, results)
            if err:
                return err
