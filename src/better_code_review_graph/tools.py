...
def query_graph(
    ...
    try:
        ...
        qn = node.qualified_name if node else resolved_target
        results: list[dict] = []
        edges_out: list[dict] = []

        if pattern == "callers_of":
            _handle_callers_of(store, qn, node, results, edges_out)
        elif pattern == "callees_of":
            _handle_callees_of(store, qn, results, edges_out)
        elif pattern == "imports_of":
            _handle_imports_of(store, qn, results, edges_out)
        elif pattern == "importers_of":
            err = _handle_importers_of(store, root, node, target, results, edges_out)
            if err:
                return err
        elif pattern == "children_of":
            _handle_children_of(store, qn, results, edges_out)
        elif pattern == "tests_for":
            _handle_tests_for(store, qn, node, target, results, edges_out)
        elif pattern == "inheritors_of":
            _handle_inheritors_of(store, qn, results, edges_out)
        elif pattern == "file_summary":
            err = _handle_file_summary(store, root, target, results, edges_out)
            if err:
                return err
...
def _resolve_query_target(
    ...
    """Resolve query target to a node. Returns (node, target, error_response)."""
