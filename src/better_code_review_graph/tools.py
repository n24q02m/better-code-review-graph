...
def _resolve_query_target(
    ...
    """Resolve a query target string to a Node object.

    Try as-is, then as absolute path, then search by name.
    Returns (node, resolved_target_name, error_response).
    """
...
def _fetch_nodes_to_dicts(store: GraphStore, qns: list[str]) -> list[dict[str, Any]]:
    """Fetch multiple nodes by qualified name and convert to dicts."""
    if not qns:
        return []
...
def _handle_callees_of(store: GraphStore, qn: str) -> tuple[list[dict], list[dict]]:
    qns = []
    edges_out = []
...
def _handle_imports_of(store: GraphStore, qn: str) -> tuple[list[dict], list[dict]]:
    results = []
    edges_out = []
    for e in store.get_edges_by_source(qn):
...
def _handle_children_of(store: GraphStore, qn: str) -> tuple[list[dict], list[dict]]:
    qns = []
    for e in store.get_edges_by_source(qn):
        if e.kind == "CONTAINS":
            qns.append(e.target_qualified)
...
def _handle_inheritors_of(store: GraphStore, qn: str) -> tuple[list[dict], list[dict]]:
    qns = []
    edges_out = []
    for e in store.get_edges_by_target(qn):
...
def query_graph(
    ...
    try:
        ...
        node, target, error_res = _resolve_query_target(store, target, root)
        if error_res:
            return error_res
        ...
        if pattern == "callers_of":
            ...
            results, edges_out, error_res = _handle_importers_of(
                store, target, node, root
            )
            if error_res:
                return error_res
            ...
            results, edges_out, error_res = _handle_file_summary(store, target, root)
            if error_res:
                return error_res
