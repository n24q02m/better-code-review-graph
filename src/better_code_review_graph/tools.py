...
def _resolve_query_target(
    store: Any, root: Path, target: str, pattern: str
) -> tuple[Any | None, str, dict[str, Any] | None]:
    """Resolve the target to a node or absolute path.

    Returns:
        tuple: (node, resolved_target_name, error_response)
    """
...
def _handle_callers_of(
    store: Any, node: Any | None, qn: str, target: str
...
def _handle_callees_of(store: Any, qn: str) -> tuple[list[dict], list[dict]]:
    results = []
    edges_out = []
    qns = []
...
def _handle_imports_of(store: Any, qn: str) -> tuple[list[dict], list[dict]]:
    results = []
    edges_out = []
    for e in store.get_edges_by_source(qn):
...
def _handle_importers_of(
    store: Any, root: Path, node: Any | None, target: str
...
def _handle_children_of(store: Any, qn: str) -> list[dict]:
    results = []
    qns = []
    for e in store.get_edges_by_source(qn):
        if e.kind == "CONTAINS":
...
def _handle_tests_for(store: Any, node: Any | None, qn: str, target: str) -> list[dict]:
    results = []
    qns = []
    for e in store.get_edges_by_target(qn):
        if e.kind == "TESTED_BY":
...
def _handle_inheritors_of(store: Any, qn: str) -> tuple[list[dict], list[dict]]:
    results = []
    edges_out = []
    qns = []
    for e in store.get_edges_by_target(qn):
...
def _handle_file_summary(
    store: Any, root: Path, target: str
...
def query_graph(
    ...
    try:
        ...
        if pattern == "callers_of":
            results, edges_out = _handle_callers_of(store, node, qn, target)
        elif pattern == "callees_of":
            results, edges_out = _handle_callees_of(store, qn)
        elif pattern == "imports_of":
            results, edges_out = _handle_imports_of(store, qn)
        elif pattern == "importers_of":
            results, edges_out, error_resp = _handle_importers_of(
                store, root, node, target
            )
            if error_resp:
                return error_resp
        elif pattern == "children_of":
            results = _handle_children_of(store, qn)
        elif pattern == "tests_for":
            results = _handle_tests_for(store, node, qn, target)
        elif pattern == "inheritors_of":
            results, edges_out = _handle_inheritors_of(store, qn)
        elif pattern == "file_summary":
            results, error_resp = _handle_file_summary(store, root, target)
            if error_resp:
                return error_resp
