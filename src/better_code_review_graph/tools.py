...
def _resolve_query_target(target: str, store, root: Path) -> Any | None:
    """Resolve query target to a GraphNode."""
...
def _fetch_nodes_to_dicts(store, qns: list[str]) -> list[dict]:
    """Batch fetch nodes by qualified names and convert to dicts, preserving order."""
    if not qns:
        return []
...
def _handle_callers_of(
    ...
    return _fetch_nodes_to_dicts(store, qns), edges_out


def _handle_callees_of(qn: str, store) -> tuple[list[dict], list[dict]]:
    qns = []
    edges_out = []
...
def _handle_imports_of(qn: str, store) -> tuple[list[dict], list[dict]]:
    results = []
    edges_out = []
    for e in store.get_edges_by_source(qn):
...
def _handle_importers_of(
    target: str, node: Any | None, store, root: Path
...
def _handle_children_of(qn: str, store) -> list[dict]:
    qns = []
    for e in store.get_edges_by_source(qn):
        if e.kind == "CONTAINS":
            qns.append(e.target_qualified)
    return _fetch_nodes_to_dicts(store, qns)


def _handle_tests_for(qn: str, target: str, node: Any | None, store) -> list[dict]:
    qns = []
    for e in store.get_edges_by_target(qn):
        if e.kind == "TESTED_BY":
            qns.append(e.source_qualified)
...
def _handle_inheritors_of(qn: str, store) -> tuple[list[dict], list[dict]]:
    qns = []
    edges_out = []
    for e in store.get_edges_by_target(qn):
...
def _handle_file_summary(target: str, store, root: Path) -> list[dict] | dict[str, Any]:
    full_target_raw = root / target
    full_target = full_target_raw.resolve()
    if (
        not full_target.is_relative_to(root.resolve())
...
def query_graph(
    ...
    try:
        ...
        # Resolve target
        node = _resolve_query_target(target, store, root)
        if not node and pattern != "file_summary":
            # Check if it was ambiguous before failing
            candidates = store.search_nodes(target, limit=5)
            if len(candidates) > 1:
                return {
                    "status": "ambiguous",
        ...
        if pattern == "callers_of":
            results, edges_out = _handle_callers_of(qn, node, store)
        elif pattern == "callees_of":
            results, edges_out = _handle_callees_of(qn, store)
        elif pattern == "imports_of":
            results, edges_out = _handle_imports_of(qn, store)
        elif pattern == "importers_of":
            outcome = _handle_importers_of(target, node, store, root)
            if isinstance(outcome, dict):
                return outcome
            results, edges_out = outcome
        elif pattern == "children_of":
            results = _handle_children_of(qn, store)
        elif pattern == "tests_for":
            results = _handle_tests_for(qn, target, node, store)
        elif pattern == "inheritors_of":
            results, edges_out = _handle_inheritors_of(qn, store)
        elif pattern == "file_summary":
            outcome = _handle_file_summary(target, store, root)
            if isinstance(outcome, dict):
                return outcome
            results = outcome
