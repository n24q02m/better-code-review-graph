...
def _resolve_query_target(
    target: str, store: Any, root: Path
) -> tuple[Any | None, str]:
    """Resolve target to a node and its qualified name."""
    ...
    return node, node.qualified_name


def _handle_callers_of(
    qn: str, node: Any | None, store: Any, results: list[dict], edges_out: list[dict]
):
    qns = []
...
def _handle_callees_of(qn: str, store: Any, results: list[dict], edges_out: list[dict]):
    qns = []
...
def _handle_imports_of(qn: str, store: Any, results: list[dict], edges_out: list[dict]):
    for e in store.get_edges_by_source(qn):
...
def _handle_children_of(qn: str, store: Any, results: list[dict]):
    qns = []
    for e in store.get_edges_by_source(qn):
        if e.kind == "CONTAINS":
            qns.append(e.target_qualified)
...
def _handle_tests_for(
    qn: str, target: str, node: Any | None, store: Any, results: list[dict]
):
    qns = []
    for e in store.get_edges_by_target(qn):
        if e.kind == "TESTED_BY":
...
def _handle_inheritors_of(
    qn: str, store: Any, results: list[dict], edges_out: list[dict]
):
    qns = []
    for e in store.get_edges_by_target(qn):
...
def _handle_file_summary(
    target: str, store: Any, root: Path, results: list[dict]
) -> str | None:
    full_target_raw = root / target
    full_target = full_target_raw.resolve()
    if (
...
def query_graph(
    ...
    try:
        ...
        # Resolve target
        node, qn_or_err = _resolve_query_target(target, store, root)
        if qn_or_err == "ambiguous":
            candidates = store.search_nodes(target, limit=5)
            return {
                "status": "ambiguous",
        ...
        qn = qn_or_err if node else target

        # Dispatch
        if pattern == "callers_of":
            _handle_callers_of(qn, node, store, results, edges_out)
        elif pattern == "callees_of":
            _handle_callees_of(qn, store, results, edges_out)
        elif pattern == "imports_of":
            _handle_imports_of(qn, store, results, edges_out)
        elif pattern == "importers_of":
            err = _handle_importers_of(target, node, store, root, results, edges_out)
            if err:
                return {"status": "error", "summary": err}
        elif pattern == "children_of":
            _handle_children_of(qn, store, results)
        elif pattern == "tests_for":
            _handle_tests_for(qn, target, node, store, results)
        elif pattern == "inheritors_of":
            _handle_inheritors_of(qn, store, results, edges_out)
        elif pattern == "file_summary":
            err = _handle_file_summary(target, store, root, results)
            if err:
                return {"status": "error", "summary": err}
