...
) -> tuple[GraphNode | None, str | dict]:
    """Resolve the query target to a node and/or qualified name."""
...
def _handle_callers_of(
    qn: str, node: GraphNode | None, store: GraphStore
...
def _handle_callees_of(qn: str, store: GraphStore) -> tuple[list[dict], list[dict]]:
    results = []
    edges_out = []
    qns = []
...
def _handle_imports_of(qn: str, store: GraphStore) -> tuple[list[dict], list[dict]]:
    results = []
    edges_out = []
    for e in store.get_edges_by_source(qn):
...
def _handle_importers_of(
    target: str, node: GraphNode | None, store: GraphStore, root: Path
...
def _handle_tests_for(
    qn: str, target: str, node: GraphNode | None, store: GraphStore
) -> list[dict]:
    results = []
    qns = []
    for e in store.get_edges_by_target(qn):
...
def _handle_inheritors_of(qn: str, store: GraphStore) -> tuple[list[dict], list[dict]]:
    results = []
    edges_out = []
    qns = []
    for e in store.get_edges_by_target(qn):
