...
class CodeParser:
    ...
    def _get_bases(self, node, language: str, source: bytes) -> list[str]:
        ...
        if language in ("java", "csharp", "kotlin"):
            return self._get_bases_java_like(node)
        if language == "cpp":
            return self._get_bases_cpp(node)
        if language in ("typescript", "javascript", "tsx"):
            return self._get_bases_js_like(node)
        if language == "solidity":
    ...
    def _get_bases_java_like(self, node) -> list[str]:
        bases = []
    ...
    def _get_bases_js_like(self, node) -> list[str]:
        bases = []
        # extends clause
        for child in node.children:
