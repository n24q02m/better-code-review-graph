...
class CodeParser:
    ...
    def _get_bases(self, node, language: str, source: bytes) -> list[str]:
        ...
        if language in ("java", "csharp", "kotlin"):
            return self._get_bases_java_csharp_kotlin(node)
        if language == "cpp":
            ...
            if child.type == "import_spec_list":
                for spec in child.children:
                    if spec.type == "import_spec":
                        imports.extend(self._process_go_import_spec(spec))
            elif child.type == "import_spec":
                imports.extend(self._process_go_import_spec(child))
        return imports

    def _process_go_import_spec(self, node) -> list[str]:
        """Extract path from a Go import_spec node."""
        paths = []
