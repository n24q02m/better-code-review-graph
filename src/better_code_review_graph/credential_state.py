...
def resolve_credential_state() -> CredentialState:
    ...
    if _is_http_mode():
        try:
            ...
            logger.debug(f"Failed to load credentials from per-plugin store: {e}")
        ...
        try:
            ...
            logger.debug(f"Failed to check local mode marker: {e}")
...
def reset_state() -> None:
    ...
    try:
        ...
        logger.warning(f"Failed to clear credentials from per-plugin store: {e}")
