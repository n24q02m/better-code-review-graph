...
def get_db_path(repo_root: Path) -> Path:
    ...
    analyzed repository at ``<repo_root>/.better-code-review-graph/graph.db``.
    Creates the package-owned directory and an inner ``.gitignore`` (with
    ``*``) so generated files are never committed. Older ``.code-review-graph``
    paths are ambiguous shared state: leave them and their SQLite sidecars
    untouched, and build a fresh graph rather than migrating another package's DB.
    ...
    if not inner_gitignore.exists():
        inner_gitignore.write_text(
            "# Auto-generated by better-code-review-graph — do not commit database files.\n"
...
def find_dependents(store: GraphStore, file_path: str) -> list[str]:
    ...
    # Bolt optimization: push edge filtering to SQLite to avoid unnecessary row materialization
    ...
    for i, rel_path in enumerate(files, 1):
        ...
        try:
            n_nodes, n_edges = _update_single_file(
                full_path, parser, store, repo_registry=registry
            )
            total_nodes += n_nodes
            total_edges += n_edges
...
def _update_files_in_store(
    ...
    registry = repo_registry or RepoRegistry(store)
    total_nodes = 0
    total_edges = 0
...
def incremental_update(
    ...
    *,
    ...
    repo_registry:
        ...
        bumped to the current ``HEAD`` once indexing completes. Parsed nodes
        and edges retain their registered repository scope, including files
        first seen during this update. Without an argument, tagging uses the
        store's existing registry; unregistered files retain the single-repo scope.
    """
    parser = CodeParser()
    ignore_patterns = _load_ignore_patterns(repo_root)
...
def _update_single_file(
    ...
    *,
    ...
    if repo_registry is not None:
        try:
            repo_id = repo_registry.assign(abs_path)
        except ValueError:
            repo_id = ""
        for node in nodes:
...
class GraphUpdateHandler(FileSystemEventHandler):
    ...
    def __init__(
        ...
        self._repo_registry = RepoRegistry(store)
    ...
    def _update_file(self, abs_path: str):
        ...
        try:
            n_nodes, n_edges = _update_single_file(
                path, self.parser, self.store, repo_registry=self._repo_registry
            )
            self.store.resolve_php_calls()
