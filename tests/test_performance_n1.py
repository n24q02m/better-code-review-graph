...
from src.better_code_review_graph.parser import NodeInfo, EdgeInfo
...
def test_query_graph_n1_performance(tmp_path):
    ...
    with GraphStore(db_path) as store:
        ...
        parent = NodeInfo(kind="Function", name="parent", file_path="parent.py", line_start=1, line_end=10, language="python")
        store.upsert_node(parent)
        for i in range(500):
            child = NodeInfo(kind="Function", name=f"child{i}", file_path="child.py", line_start=1, line_end=10, language="python")
            store.upsert_node(child)
            edge = EdgeInfo(kind="CONTAINS", source="parent.py::parent", target=f"child.py::child{i}", file_path="child.py", line=1, extra={})
    ...
    with patch('src.better_code_review_graph.tools._get_store') as mock_get_store:
        ...
        start = time.time()
        result = query_graph("children_of", "parent.py::parent", repo_root=str(tmp_path))
