...
def _stub_local_model():
    """Build a stub local model that returns deterministic vectors.

    Mirrors the local inference fixture in test_embeddings.py so the
...
class TestGetDocsSection:
    ...
    def test_get_docs_section_store_init_exception_fallback(self, tmp_path):
        ...
        docs_file.write_text(
            '<section name="usage">Fallback content works!</section>',
            encoding="utf-8",
        )
